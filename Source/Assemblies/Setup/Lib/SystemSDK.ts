...
export class SystemSDK {
	public static async ConfigureServer(
		...
		try {
			...
			SystemSDK.UseExpressRouter(
				options.Application,
				options.RoutingOpts
			);
			if (options.AllowRoutes) {
				await SystemSDK.MapRoutesInternal(
					options.Application,
			...
			Logger.Error(
				`Error occurred when configuring a site! Stack: %s`,
				e.stack
			);
		}
	}
	...
	public static StartServer(
		...
		try {
			...
			if (options.UseSsl) {
				...
				baselineSslServer = (options.UseSslV2 ? SslV2 : Ssl)
					...
					.listen(options.SslPort, options.SiteName, () =>
						Logger.Info(
							`BaseLineSslServer '%s' started on port %d.`,
							options.SiteName,
							options.SslPort
						)
					);
			...
			if (options.UseInsecure)
				baseLineServer = options.Application.listen(
					options.InsecurePort,
					options.SiteName,
					() =>
						Logger.Info(
							`BaseLineServer '%s' started on port %d.`,
							options.SiteName,
							options.InsecurePort
						)
				);
	...
	private static async MapRoutesInternal(
		application?: IExpressApplication,
		options?: ISiteRouteSetupOptions
	): Promise<void> {
		return new Promise(async (resumeFunction) => {
			const directory =
				(options !== undefined
					? options.RouteStorePath
					: __baseDirName + "/Routing") || __baseDirName + "/Routing";
					...
					if (map) {
						...
						try {
							if (requestMethod === "get" && !isMiddleware) {
								...
								if (isMiddleware) {
									Logger.Debug(
										`Mapping 'ALL ROUTE CATCHER' for the site '%s'`,
										options.SiteName
									);
						...
						return Logger.Warn(
							"The route '%s' for the site '%s' had no default export.",
							file,
							options.SiteName
						);
					}
	...
	private static UseExpressRouter(
		app: IExpressApplication,
		opts?: IRoutingOptions
	): void {
		app.use(Router(opts));
	}
}
