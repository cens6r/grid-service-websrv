...
import {
	hostname as GetCurrentMachineName,
...
export class HttpClient {
	 ...
	 */
	public async ExecuteAsync<TResponse = any>(): Promise<
		[boolean, ClientResponse<TResponse>, Error]
	> {
		return new Promise<[boolean, ClientResponse, Error]>(
			(resumeFunction) => {
				if (!this.request)
					return resumeFunction([
						false,
						null,
						new TypeError(
							"The request was null, please update it via 'UpdateConfiguration'."
						),
					]);

				const parsedQs = new URLSearchParams(this.request.QueryString).toString();
